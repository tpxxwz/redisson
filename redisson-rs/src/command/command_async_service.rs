...
/// 对应 Java EVALSHA_SUPPORTED：Redis >= 7.0 支持 EVALSHA（早期版本无此命令）
...
// ============================================================
// 内部执行方法 — 对应 Java CommandAsyncService.async() / evalAsync()
// （自由函数，避免 async fn(&self) 在 trait impl async move 块中的生命周期问题）
// ============================================================

/// 对应 Java async(boolean readOnlyMode, ...) 的核心逻辑
/// 处理 SORT_RO fallback
///
/// TODO(ignoreRedirect): Java 的 ignoreRedirect 参数防止 Batch 模式下命令跟随 MOVED/ASK redirect 分裂到不同节点。
///   fred 的 Pool 没有暴露 ignoreRedirect 选项，batch 模式下用 ClusterHash::Custom 仍有跟随 redirect 分裂的风险。
    ...
    T: FromValue,
{
    ...
    if read_only_mode && command.name == "SORT" {
        if !SORT_RO_SUPPORTED.load(Ordering::Relaxed) {
            ...
            // flag=true 时尝试 SORT_RO
            let sort_ro_cmd = CustomCommand::new_static("SORT_RO", slot.clone(), false);
            match exec(&pool, true, sort_ro_cmd, all_args.clone(), command.convertor).await {
                Ok(v) => return Ok(v),
                Err(e) if e.to_string().starts_with("ERR unknown command") => {
                    SORT_RO_SUPPORTED.store(false, Ordering::Relaxed);
                    // 降级：走普通 SORT（readOnlyMode 保持 false）
                    let sort_cmd = CustomCommand::new_static(command.name, slot, false);
                    return exec(&pool, false, sort_cmd, all_args, command.convertor).await;
                }
                Err(e) => return Err(e),
            }
        }
    ...
    exec(&pool, read_only_mode, cmd, all_args, command.convertor).await
}
...
async fn exec<T>(
    ...
    cmd: CustomCommand,
    all_args: Vec<Value>,
    convertor: Option<&'static dyn crate::client::protocol::convertor::Convertor<T>>,
) -> Result<T>
where
    T: FromValue,
{
...
/// 对应 Java evalAsync(NodeSource, boolean readOnlyMode, ...) 的核心逻辑：
    ...
    T: FromValue,
{
    ...
    if !use_script_cache {
        let all_args = build_eval_args(script, lua_keys, args_vec);
        let cmd = CustomCommand::new_static(command.name, slot, false);
        return exec(&pool, read_only_mode, cmd, all_args, command.convertor).await;
    }
    // 构建 EVAL/EVALSHA args：[sha/script, numkeys, key..., arg...]
    ...
    if read_only_mode && EVALSHA_RO_SUPPORTED.load(Ordering::Relaxed) {
        ...
        match exec(&pool, true, cmd, all_args, command.convertor).await {
            Ok(v) => return Ok(v),
            Err(e) if e.to_string().starts_with("ERR unknown command") => {
    ...
    if EVALSHA_SUPPORTED.load(Ordering::Relaxed) {
        ...
        match exec(&pool, read_only_mode, cmd, all_args, command.convertor).await {
            Ok(v) => return Ok(v),
            Err(e) if e.to_string().starts_with("ERR unknown command") => {
                EVALSHA_SUPPORTED.store(false, Ordering::Relaxed);
            }
            Err(e) if e.to_string().starts_with("NOSCRIPT") => {
                // script 未缓存，先 SCRIPT LOAD 再重试
                let sha = script_load(&pool, &slot, &script).await?;
                let all_args = build_eval_all_args(&sha);
...
async fn script_load(pool: &Pool, slot: &ClusterHash, script: &str) -> Result<String> {
    ...
    let raw: Value = pool.custom(cmd, all_args).await?;
    let s = <String as FromValue>::from_value(raw)?;
    Ok(s)
}
...
impl CommandAsyncExecutor for CommandAsyncService {
        ...
        R::Error: Into<Error> + Send,
    {
        ...
        async move {
            let all_args = build_args(command.sub_name, args)?;
            async_inner(pool, use_replica, slot, command, all_args).await
        }
    }
        ...
        R::Error: Into<Error> + Send,
    {
        ...
        async move {
            let all_args = build_args(command.sub_name, args)?;
            async_inner(pool, false, slot, command, all_args).await
        }
    }
        ...
        R::Error: Into<Error> + Send,
    {
        ...
        async move {
            eval_async_inner(pool, false, slot, command, script, lua_keys, args_result?.into_array(), use_script_cache).await
        }
    }
        ...
        R::Error: Into<Error> + Send,
    {
        ...
        let use_script_cache = self.is_eval_cache_active();
        async move {
            eval_async_inner(pool, use_replica, slot, command, script, lua_keys, args_result?.into_array(), use_script_cache).await
        }
    }
}
